...
private:
  ...
  void twist_cb(const geometry_msgs::msg::Twist::SharedPtr twist_msg)
  {
    ...
    v_r = ((max_rps + v_r) / max_rps) - 1.0;
    v_l = ((max_rps + v_l) / max_rps) - 1.0;

    // Other parameter modifications to motor
    float pow_reduce = 0.5;
    // float trim_lr_factor = 1.0;

    std_msgs::msg::Float32 pow_r;
    std_msgs::msg::Float32 pow_l;
    pow_r.data = pow_reduce * std::min(std::max(v_r, -1.0), 1.0);
    pow_l.data = pow_reduce * std::min(std::max(v_l, -1.0), 1.0);
