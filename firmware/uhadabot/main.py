...
class RangeSensorSet:
    ...
    def __init__(self, ros):
        self.ros = ros
        self.ros_rs_pub = Topic(
            ros, "hadabot/ir_sensors_12bit_adc",
        ...
        for rs_pc in PIN_CONFIG["range_sensors"]:
            ...
            rs_adc.width(
                ADC.WIDTH_12BIT)  # 12bits

            rs_msg_label += (rs_pc["label"] + "_")
            self.r_sensors.append({
                "label": rs_pc["label"],
                "adc_pin": rs_adc,
                "fov_half_radians": (3.0/360.0) * 6.28
            })
        rs_msg_label = rs_msg_label.strip(
            "_")  # Remove trailing _

        # The multiarray int32 ros message used to publish
        # the 12-bit sensor ADC readings
        self.ros_ir_sensor_message_json = {
            "layout": {
                "dim": [{
                    "label": rs_msg_label,
                    "size": len(self.r_sensors),
                    "stride": len(self.r_sensors)
                }],
                "data_offset": 0,
            },
            "data": [0] * len(self.r_sensors)
        }

    def publish_range_m(self):
        for idx, rs in enumerate(self.r_sensors):
            val_12bit = rs["adc_pin"].read()
            self.ros_ir_sensor_message_json["data"][idx] = val_12bit

        # logger.info("IR pub {}".format(self.ros_ir_sensor_message_json))
        self.ros_rs_pub.publish(
            Message(self.ros_ir_sensor_message_json))
...
class Encoder:
    ...
    def en_cb(self, pin):
        # 1 if fwd, -1 if rev
        self.count += self.spin_dir_fwd_rev
...
class EncoderSet:
    ...
    def publish_encoders(
        ...
        self.ros_pub.publish(
            Message(self.ros_encoders_message_json))
    ...
    def set_spin_direction(
            self, wheel_power, channel_idx):
...
class Controller:
    ...
    def __init__(self, ros):
        ...
        # Motor pins
        self.pwm_pin_left = PWM(
            Pin(PIN_CONFIG["left"]["motor"]["pwm"]))
        self.pwm_pin_right = PWM(
            Pin(PIN_CONFIG["right"]["motor"]["pwm"]))

        self.fr_pin_left = Pin(
            PIN_CONFIG
            ["left"]
            ["motor"]["fr"],
            Pin.OUT)
        self.fr_pin_right = Pin(
            PIN_CONFIG
            ["right"]
            ["motor"]["fr"],
            Pin.OUT)
    ...
    ###########################################################################
    def turn_wheel(
        self, wheel_power_f32, pwm_pin,
            fr_pin, prev_direction):
        factor = max(
            min(wheel_power_f32, 1.0), -1.0)

        # Reversing direction? Stop first!
        if (prev_direction * wheel_power_f32) < 0.0:
            self._send_motor_signal(
                0.0, pwm_pin, fr_pin)
            time.sleep_ms(10)

        # Send command
        self._send_motor_signal(
            factor, pwm_pin, fr_pin)

    ###########################################################################
    def _send_motor_signal(
            self, factor, pwm_pin, fr_pin):
        if factor >= 0:
            ...
            fr_pin.on()
            pwm_pin.duty(
                1023 - int(1023 * (-1*factor)))

    ###########################################################################
    def right_wheel_cb(self, wheel_power):
        left_right = 1  # right
        spin_dir = self.en_set.encoder_list[left_right].spin_dir_fwd_rev
        self.en_set.set_spin_direction(
            wheel_power["data"], left_right)
        self.turn_wheel(
            wheel_power["data"],
            self.pwm_pin_right, self.fr_pin_right,
            spin_dir)
    ...
    def left_wheel_cb(self, wheel_power):
        ...
        self.turn_wheel(
            wheel_power["data"],
            self.pwm_pin_left, self.fr_pin_left,
            spin_dir)
    ...
    def run_once(self):
        ...
        # Publish radps
        self.en_set.publish_encoders(
            [count_left, count_right])

        if time.ticks_diff(
                cur_ms, self.last_rs_pub_ms) > 500:
            # Publish range data
            self.rs_set.publish_range_m()
            self.last_rs_pub_ms = cur_ms

        if False:
            logger.info("Encoder count {} - {}".format(
...
class Hadabot:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.blink_led_sub_topic = Topic(
                self.ros, "hadabot/blink_led",
                "std_msgs/Int32")
            self.blink_led_sub_topic.subscribe(
                self.blink_led_cb)
            ...
            self.rw_sub_topic = Topic(
                ...
                "std_msgs/Float32")
            self.rw_sub_topic.subscribe(
                self.controller.right_wheel_cb)
            self.lw_sub_topic = Topic(
                self.ros,
                "hadabot/wheel_power_left",
                "std_msgs/Float32")
            self.lw_sub_topic.subscribe(
                self.controller.left_wheel_cb)
            ...
            self.boot_button = Pin(
                PIN_CONFIG
                ["button"]
                ["boot"],
                Pin.IN)
            self.boot_button.irq(
                trigger=Pin.IRQ_RISING,
    ...
    def ping_time_reference_cb(self, msg):
        self.ping_time_reference_ack_topic.publish(
            Message(msg))
        self.last_hb_ms = time.ticks_ms()

    ###########################################################################
    def ping_inertia_stamped_cb(self, msg):
        self.ping_inertia_stamped_ack_topic.publish(
            Message(msg))
        self.last_hb_ms = time.ticks_ms()
        ...
        while go:
            try:
                ...
                if time.ticks_diff(cur_ms, self.last_hb_ms) > 5000:
                    ...
                    self.log_info.publish(
                        Message(str(cur_ms)))
                    self.last_hb_ms = cur_ms

            except Exception as ex:
                logger.error(
                    "Spin exception hit {}".format(str(ex)))
                go = False
